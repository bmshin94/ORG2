...
impl Connection {
    ...
    pub async fn access(self: &Arc<Self>) -> Result<AccessCredential, &'static str> {
        ...
        self.access_task(move |grant| async move { owner.renew(grant).await })
            .await
    }
...
pub(crate) mod tests {
    ...
    async fn concurrent_consumers_share_rotation_and_cached_access() {
        ...
        let renew = |mut g: Grant| async {
            ...
            g.token = "og2ms.v1.rotated".into();
            Ok(g)
        };
        let (a, b) = tokio::join!(c.access_with(renew), c.access_with(renew));
        assert_eq!(a.unwrap().bearer(), "og2ms.v1.rotated");
        assert_eq!(b.unwrap().bearer(), "og2ms.v1.rotated");
