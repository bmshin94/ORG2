...
const CONSOLE: &str = "https://market.org2.dev";
...
impl Target {
    ...
    pub fn harness_name(self) -> Option<&'static str> {
        match self {
            Self::ClaudeCode => Some("claude_code"),
            Self::ClaudeDesktop => Some("claude_desktop"),
...
impl Enrollment {
    ...
    pub fn begin(&mut self, selection: Selection) -> Result<String, &'static str> {
...
mod tests {
    ...
    #[test]
    fn approved_targets_have_explicit_mappings() {
        assert_eq!(Target::ClaudeDesktop.harness_name(), Some("claude_desktop"));
        assert_eq!(Target::Org2.harness_name(), None);
    }
    #[test]
