...
fn durable_credential_source_is_projected_by_every_session_read() {
    ...
    conn.execute("UPDATE code_sessions SET runner='tui', account_id=NULL WHERE session_id=?1", [session_id]).unwrap();
    bind_credential_source(session_id, "test:workspace-a", "claude_code", "claude-sonnet-4-6").unwrap();
    create_test_session("cli-ordinary-projection", "account-b");
    let assert_source = |rows: Vec<CodeSession>| {
        let row = rows.iter().find(|row| row.session_id == session_id).unwrap();
...
fn execution_creation_commits_source_with_owner_or_neither() {
    ...
    let created = create_session_with_source("cli-atomic-source", &params, Some("test:workspace")).unwrap();
    assert_eq!(created.credential_source.as_deref(), Some("test:workspace"));
    assert_eq!(created.runner, "local");
    for (id, patch, source) in [
        ("cli-mixed-source", serde_json::json!({"accountId":"keyvault"}), "test:workspace"),
        ("cli-wrong-runner", serde_json::json!({"runner":"tui"}), "test:workspace"),
        ("cli-empty-source", serde_json::json!({}), ""),
    ] {
        let mut raw = serde_json::json!({"platform":"codex", "model":"model", "repoPath":"/tmp"});
        raw.as_object_mut().unwrap().extend(patch.as_object().unwrap().clone());
