            ...
            for after_write in [false, true] {
                ...
                let run = |inject: bool| {
                    let step = |stage: usize, effect: &dyn Fn() -> Result<(), String>| {
                        if inject && failed_stage == stage && !after_write { return Err("injected failure".into()); }
                        effect()?;
                        if inject && failed_stage == stage && after_write { return Err("injected failure".into()); }
                        Ok(())
                    };
                    disconnect_steps(
                        ...
                        || step(2, &|| agent_cli::managed_config::write_cli_profile_file_atomic(&index, br#"["other"]"#)),
                    )
                };
                assert!(run(true).is_err());
                if failed_stage == 0 { assert!(grant.exists()); }
